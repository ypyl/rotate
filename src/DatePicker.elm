...
import Colors exposing (black, gray, white)
...
dayView today month year =
    ...
    column [ Background.color white, Border.color black, Border.width 1, Border.rounded 5, spacing spacingValue, padding 5 ]
        ([ row [ width fill ]
         ...
         , row [ spacing spacingValue, width fill ] (generateDaysRow initialDate month)
         ]
            ++ generateRowsForMonth month (add Days 7 initialDate)
        )
...
monthView today year =
    let
        monthBtn m =
            button [ width (px 60) ] { onPress = Just (ShowDayView m year), label = el [ centerX ] (text (monthToShortStr m)) }
    in
...
yearView today year =
    let
        ...
        yearBtn y =
            button [ width (px 60) ] { onPress = Just (ShowMonthView y), label = el [ centerX ] (text (String.fromInt y)) }
    in
    column [ Background.color white, Border.color black, Border.width 1, Border.rounded 5, spacing 20, padding 5 ]
        [ row [ width fill ]
            [ el [ width (px cellWidth), alignLeft ] (button [ width fill ] { onPress = Just Previous, label = el [ centerX ] (text "<") })
...
generateDaysRow : Date -> Month -> List (Element Msg)
generateDaysRow date currentMonth =
    let
        txtColor d =
...
generateRowsForMonth : Month -> Date -> List (Element Msg)
generateRowsForMonth monthValue startDate =
    let
        internalGenerator dateValue result =
            if month dateValue == monthValue then
                row [ spaceEvenly, width fill ] (generateDaysRow dateValue monthValue) :: internalGenerator (add Days 7 dateValue) result
